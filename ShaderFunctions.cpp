...
GLuint makeComputeShaderProgram(GLuint computeShaderID)
{
	GLuint programid = glCreateProgram();
...
{
	char* computeShaderSourceCode = readFile(compute_shader_file_name);
	GLuint computeShaderID = makeShader(computeShaderSourceCode, GL_COMPUTE_SHADER);
	GLuint computeShaderProgramID = makeComputeShaderProgram(computeShaderID);
	printf("\nCompute Shader ID:%d", computeShaderID);
	printf("\ncomputeShaderProgramID:%d", computeShaderProgramID);
